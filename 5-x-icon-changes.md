## Pack Icon Changes 5.2.1 => 5.3.0
### New icons (7)
- AirplaneSuccess
- ArrowDownDiamondOutline
- ArrowLeftDiamondOutline
- ArrowRightDiamondOutline
- ArrowUpDiamondOutline
- NonProfit
- WeatherClearNight
### Icons with visual changes (0)
_None_
### Removed icons (1)
- AirplaceSuccess

## Pack Icon Changes 5.2.0 => 5.2.1
### New icons (0)
_None_
### Icons with visual changes (0)
_None_
### Removed icons (0)
_None_

## Pack Icon Changes 5.1.0 => 5.2.0
### New icons (0)
_None_
### Icons with visual changes (0)
_None_
### Removed icons (0)
_None_

## Pack Icon Changes 5.0.0 => 5.1.0
### New icons (11)
- AccountVoiceOffOutline
- AccountVoiceOutline
- CalendarRemoveMultiple
- CarTrunk
- Chicken
- CoffeeBean
- CrossCircle
- CrossCircleOutline
- DomainArrowRight
- RadiantHeat
- WindowClosedBroken
### Icons with visual changes (2)
- RocketLaunch
- RocketLaunchOutline
### Removed icons (0)
_None_

## Pack Icon Changes 4.9.0 => 5.0.0
### New icons (161)
- AccountBoxEditOutline
- AccountBoxMinusOutline
- AccountBoxPlusOutline
- AccountFile
- AccountFileOutline
- AccountFileText
- AccountFileTextOutline
- BookOpenBlankVariantOutline
- BookOpenVariantOutline
- BriefsOutline, UnderwearOutline
- BusSign
- BusWrench
- CashEdit
- ChartBoxMultiple, GraphBoxMultiple, PollBoxMultiple, ReportBoxMultiple, WidgetBoxMultiple
- ChartBoxMultipleOutline, GraphBoxMultipleOutline, PollBoxMultipleOutline, ReportBoxMultipleOutline, WidgetBoxMultipleOutline
- Conversation, Dialogue, Discussion
- ConversationMinus, DialogueMinus, DiscussionMinus
- ConversationMinusOutline, DialogueMinusOutline, DiscussionMinusOutline
- ConversationOutline, DialogueOutline, DiscussionOutline
- ConversationPlus, DialoguePlus, DiscussionPlus
- ConversationPlusOutline, DialoguePlusOutline, DiscussionPlusOutline
- ConversationRemove, DialogueRemove, DiscussionRemove
- ConversationRemoveOutline, DialogueRemoveOutline, DiscussionRemoveOutline
- DiaperOutline, NappyOutline
- DownloadMultipleOutline
- EmoticonMinus
- EmoticonMinusOutline
- EmoticonPlus
- EmoticonPlusOutline
- EmoticonRemove
- EmoticonRemoveOutline
- EthernetOff
- FireStation
- GasStationInUse
- GasStationInUseOutline
- HighTunnel
- InvoiceExportOutline, InvoiceOutboundOutline
- InvoiceFast
- InvoiceFastOutline
- InvoiceImport, InvoiceInbound
- InvoiceImportOutline, InvoiceInboundOutline
- InvoiceLineItems, InvoiceList
- InvoiceLineItemsOutline, InvoiceListOutline
- InvoiceMultiple
- InvoiceMultipleOutline
- InvoiceNew
- InvoiceNewOutline, InvoicePlusOutline
- InvoiceScheduled
- InvoiceScheduledOutline
- InvoiceText
- InvoiceTextArrowLeft
- InvoiceTextArrowLeftOutline
- InvoiceTextArrowRight
- InvoiceTextArrowRightOutline
- InvoiceTextCheck
- InvoiceTextCheckOutline
- InvoiceTextClock, InvoiceTextScheduled
- InvoiceTextClockOutline, InvoiceTextScheduledOutline
- InvoiceTextEdit
- InvoiceTextEditOutline
- InvoiceTextFast
- InvoiceTextFastOutline
- InvoiceTextMinus
- InvoiceTextMinusOutline
- InvoiceTextMultiple
- InvoiceTextMultipleOutline
- InvoiceTextNew, InvoiceTextPlus
- InvoiceTextNewOutline, InvoiceTextPlusOutline
- InvoiceTextOutline
- InvoiceTextRemove
- InvoiceTextRemoveOutline
- InvoiceTextSend
- InvoiceTextSendOutline
- IslandVariant
- LinkCircle
- LinkCircleOutline
- LinkEdit
- Local, Offline
- Moonrise, WeatherMoonsetUp
- MouseLeftClick
- MouseLeftClickOutline
- MouseOutline
- MouseRightClick
- MouseRightClickOutline
- MouseScrollWheel
- PassportAlert
- PassportCancel
- PassportCheck
- PassportMinus
- PassportPlus
- PassportRemove
- Pocket
- ProgressTag
- QueueFirstInLastOut
- Slider
- SliderVertical
- SwapHorizontalHidden
- Times
- TooltipAccountOutline
- TrainBus
- TruckPickup
- UploadBox
- UploadBoxOutline
- UploadCircle
- UploadCircleOutline
- UploadMultipleOutline
- UsbCPort
- Utensils
- UtensilsClean
- UtensilsFork
- UtensilsForkKnife
- UtensilsKnife
- UtensilsSpoon
- UtensilsVariant
- Washer
- WasherAlert
- WasherOff
- WaveArrowDown
- WaveArrowUp
- WaveUndercurrent
- WeatherMoonset
- WeatherMoonsetDown
- WeatherNightOutline
### Icons with visual changes (26)
- BookOpenBlankVariant, ImportContacts
- BookOpenVariant
- ImageRemove
- Invoice
- InvoiceArrowLeft
- InvoiceArrowLeftOutline
- InvoiceArrowRight
- InvoiceArrowRightOutline
- InvoiceCheck
- InvoiceCheckOutline
- InvoiceClock
- InvoiceClockOutline
- InvoiceEdit
- InvoiceEditOutline
- InvoiceMinus
- InvoiceMinusOutline
- InvoiceOutline
- InvoicePlus
- InvoiceRemove
- InvoiceRemoveOutline
- InvoiceSend
- InvoiceSendOutline
- Lasso
- Logout, SignOut
### Removed icons (6)
- InvoiceAdd
- InvoiceReceive
- InvoiceReceiveOutline
- InvoiceSchedule, ReceiptTextPending
- InvoiceScheduleOutline

